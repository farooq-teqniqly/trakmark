...
public sealed class MeetResultTests
{
    ...
    private static readonly Event TfJumpEvent = new(Discipline.Jump("Long Jump"), Sport.TrackAndField);
    private static readonly Event TfRelayEvent = new(Discipline.RelayRun(400), Sport.TrackAndField);
    private static readonly Event TfPlaceOnlyEvent = new(Discipline.PlaceOnly("Combined Points"), Sport.TrackAndField);
    ...
    public void RecordResult_NonFinished_WithMark_Throws(ResultStatus status)
    {
        ...
        // Act / Assert
        Assert.Throws<InvalidOperationException>(() =>
            meet.RecordResult(StudentId.NewId(), TfRunEvent, status, new TimeMark(11500), place: null, null));
    }
    ...
    public void RecordResult_NonFinished_WithPlace_Throws(ResultStatus status)
    {
        ...
        // Act / Assert
        Assert.Throws<InvalidOperationException>(() =>
            meet.RecordResult(StudentId.NewId(), TfRunEvent, status, mark: null, new Placement(1), null));
    }
    ...
    public void RecordResult_MultipleResultsForSameStudent_AssignsEntryOrder()
    {
        ...
        meet.RecordResult(studentId, TfJumpEvent, ResultStatus.Finished, new DistanceMark(450), new Placement(2), null);

        // Assert
        var results = meet.Results.Where(r => r.StudentId == studentId).OrderBy(r => r.Order).ToList();
    ...
    public void RecordResult_OrderIsPerStudent_NotGlobal()
    {
        ...
        // Act
        meet.RecordResult(student1, TfRunEvent, ResultStatus.Finished, new TimeMark(11500), new Placement(1), null);
        meet.RecordResult(student2, TfRunEvent, ResultStatus.Finished, new TimeMark(12000), new Placement(2), null);
        meet.RecordResult(student1, TfJumpEvent, ResultStatus.Finished, new DistanceMark(450), new Placement(1), null);

        // Assert
        var s1Results = meet.Results.Where(r => r.StudentId == student1).OrderBy(r => r.Order).ToList();
    ...
    public void RecordResult_DistanceMarkForTimeEvent_Throws()
    {
        ...
        // Act / Assert
        Assert.Throws<InvalidOperationException>(() =>
            meet.RecordResult(StudentId.NewId(), TfRunEvent, ResultStatus.Finished, new DistanceMark(450), new Placement(1), null));
    }
    ...
    public void RecordResult_PlaceOnly_Finished_WithPlaceAndNoMark_Succeeds()
    {
        ...
        // Act
        meet.RecordResult(StudentId.NewId(), TfPlaceOnlyEvent, ResultStatus.Finished, mark: null, new Placement(1), null);
    ...
    public void RecordResult_CrossSportEvent_Throws()
    {
        ...
        // Act / Assert
        Assert.Throws<InvalidOperationException>(() =>
            meet.RecordResult(StudentId.NewId(), XcRunEvent, ResultStatus.Finished, new TimeMark(1200000), new Placement(1), null));
    }
    ...
    public void RecordResult_Relay_OneResultPerStudentWithSharedTime()
    {
        // Arrange
        var meet = CreateTrackMeet();
        var students = new[] { StudentId.NewId(), StudentId.NewId(), StudentId.NewId(), StudentId.NewId() };
        var teamTime = new TimeMark(185000);

        // Act
        foreach (var student in students)
        {
            meet.RecordResult(student, TfRelayEvent, ResultStatus.Finished, teamTime, new Placement(1), null);
        }
