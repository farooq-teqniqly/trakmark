...
- No inline XML comments on code that is self-explanatory. Add `<summary>` XML docs on all `internal` and `public` types, constructors, methods, and non-trivial fields. Exception: `[LoggerMessage]` methods in logging classes (`*.Logging.cs`) — the message template is self-documenting.
