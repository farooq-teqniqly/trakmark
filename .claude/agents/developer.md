...
- **AAA marker cleanliness**: scan every test method for `// Arrange`, `// Act`, and `// Assert` comments before staging. Each marker must appear alone on its line with no trailing text — `// Assert — some explanation` is a violation. If the phases collapse to a single expression, use `// Act / Assert` or `// Arrange / Act / Assert` on one line. Do not annotate markers with inline notes.
